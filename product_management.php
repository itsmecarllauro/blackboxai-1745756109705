...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' && $name && $price > 0) {
        $stmt = $db->prepare("INSERT INTO products (name, price, stock, category) VALUES (:name, :price, :stock, :category)");
    ...
    <main class="p-6 max-w-4xl mx-auto">
        ...
        <section class="mb-8">
            ...
            <form method="POST" class="space-y-4 bg-white p-6 rounded shadow">
                ...
                <button type="submit" class="bg-indigo-600 text-white py-2 px-4 rounded hover:bg-indigo-700 transition">Add Product</button>
            </form>
        </section>
        <section>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="p-3 space-x-2">
                            <button onclick="editProduct(<?php echo $product['id']; ?>, '<?php echo htmlspecialchars(addslashes($product['name'])); ?>', <?php echo $product['price']; ?>, <?php echo $product['stock']; ?>, '<?php echo htmlspecialchars(addslashes($product['category'])); ?>')" class="bg-yellow-400 text-white px-3 py-1 rounded hover:bg-yellow-500 transition">Edit</button>
